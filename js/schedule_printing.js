function create_schedule_table(container, events, cols){
    ...
    if(events.length>0){
        ...
        $.each(events,function(row_index,row){
            var row_dom = $("<tr/>");
            $.each(cols,function(idx,key){
                var td = $("<td/>").html(row[key] == null ? "-":row[key]).addClass("border");
                if(idx != cols.length-1){
                    td.addClass("td.fit")
                }
                row_dom.append(td);
            ...
            success: function (result) {
                ...
                $.each(dates, function(date_index,date){
                    ...
                    doc.autoTable({
                        ...
                        headStyles:{valign: 'middle',  halign : 'center', fillColor : [128,128,128], padding:2, 
                            minCellHeight:10,  lineColor: [0, 0, 0], lineWidth: 0.5},
